...
export function isManualOrEmptyKoordinat(value?: string | null): boolean {
  const trimmed = value?.trim() ?? ''
  return trimmed === '' || trimmed.toLowerCase() === 'manual'
}

export function hasParsableKoordinat(value?: string | null): boolean {
  return !isManualOrEmptyKoordinat(value) && parseKoordinatLoose(value) !== null
}